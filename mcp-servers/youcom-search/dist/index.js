...
class YoucomSearchServer {
    ...
    async searchYoucom(query, count) {
        ...
        try {
            const response = await axios.post(`${this.baseUrl}${endpoint}`, payload, { headers, timeout: 10000 });
    ...
    formatSearchResults(results, searchType, query) {
        if (results.length === 0) {
